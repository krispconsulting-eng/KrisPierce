(function () {
 ...
 // Aurora Glow redesign of the Signature Wellness Wheel (PREVIEW ONLY).
 // Same rotation/highlight engine as assets/wellness-wheel.js, recoloured
 // to match the new palette. Wire this up alongside
 // assets/wellness-wheel-aurora.css when the redesign is ready to go live;
 // see wheel-preview.html for an isolated look at it first.
 ...
 // Eight hues drawn from the Aurora Glow palette: a sweep through
 // indigo, blue-violet and cyan, with the existing clay (rose) and
 // brass (gold) accents kept for Emotional and Financial.
 var DIMS = [
   { name: 'Social', color: '#6A6FC0', tag: 'People who know you, not just who you care for.' },
   { name: 'Occupational', color: '#4F76C6', tag: 'Your work, your role, and who you still are.' },
   { name: 'Environmental', color: '#3FA8B8', tag: 'Surroundings that steady you, not just function.' },
   { name: 'Intellectual', color: '#45C6C6', tag: 'A mind that gets to stay curious, not just vigilant.' },
   { name: 'Spiritual', color: '#4B4E8F', tag: "Meaning that holds steady when things don't." },
   { name: 'Emotional', color: '#C98F97', tag: 'Naming what you feel, not just managing it.' },
   { name: 'Physical', color: '#4FAE9E', tag: "Tending your own body, not only everyone else's." },
 ...
 function drawWheel() {
  ...
  wheel.appendChild(el('circle', {
    ...
    cy: CY,
    r: INNER,
    fill: 'url(#wwPaperAurora)',
    stroke: 'rgba(231,230,248,0.95)',
    'stroke-width': '1.1'
  }));
  ...
  var markWrap = el('g', { transform: 'translate(180 180) scale(2.18) translate(-12 -12)' });
   ...
   mark.appendChild(el('circle', { cx: 12, cy: 12, r: 1.6, fill: '#2B2A55', stroke: 'none' }));
 ...
 function lift(index) {
   wedges.forEach(function (group, i) {
     if (i === index) {
       ...
       group.style.transform = 'translate(' + (u[0] * LIFT) + 'px, ' + (u[1] * LIFT) + 'px) scale(1.014)';
     } else {
 ...
 function run(step) {
  ...
  wheel.style.transition = 'transform ' + rotateDuration + 'ms cubic-bezier(.11,.7,.06,1)';
