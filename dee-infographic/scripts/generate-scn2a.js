/* SCN2A-related disorders poster -> SVG -> PNG (no browser; resvg + opentype) */
const fs = require('fs');
...
/* ---------- palette ---------- */
const C = {
  ink:'#16223A', inkSoft:'#3B4A63', muted:'#5E6A85', line:'#DFE2EC',
  paper:'#FFFFFF', bg1:'#EFF1F9', bg2:'#E7EAF5',
  brand:'#4F46E5', brandDeep:'#3730A3',
  cog:'#2563EB', cogT:'#E4EDFD',
  mot:'#0E8C9B', motT:'#DFF2F4',
  sen:'#B0700A', senT:'#FAEFD8',
  med:'#C2475C', medT:'#FBE6EA',
  beh:'#2E8B57', behT:'#E2F2E9',
  gof:'#D9821A', gofT:'#FBEEDA',
  lof:'#5B6CE6', lofT:'#E7EAFB',
  warnBg:'#FDECEF', warnBd:'#F3D6DC', tealBg:'#ECECFB', tealBd:'#D7D9F2',
};
...
const I = {
  ...
  down:'<circle cx="12" cy="12" r="9"/><path d="M12 7.5v9"/><path d="M8 12.5l4 4 4-4"/>',
  pill:'<path d="M10.5 3.8 3.8 10.5a4.2 4.2 0 0 0 6 6l6.7-6.7a4.2 4.2 0 0 0-6-6z"/><path d="M7.2 7.2l6 6"/>',
  dna:'<path d="M8 3c0 4 8 5 8 9s-8 5-8 9"/><path d="M16 3c0 4-8 5-8 9s8 5 8 9"/><path d="M9 6h6"/><path d="M9.5 9.5h5"/><path d="M9.5 14.5h5"/><path d="M9 18h6"/>',
  spectrum:'<rect x="3" y="10" width="18" height="4" rx="2"/><circle cx="7" cy="12" r="1.4"/><circle cx="17" cy="12" r="1.4"/>',
...
let hy = 40;
// eyebrow
const ebText='RARE GENETIC NEURODEVELOPMENTAL DISORDER';
const ebW = adv(ebText,12,true)+18+22;
headBuf.push(rrect(M, hy, ebW, 30, 15, 'rgba(255,255,255,0.14)', 'rgba(255,255,255,0.28)',1));
headBuf.push(icon('dna', M+11, hy+6, 18, '#EEF0FE', 1.9));
headBuf.push(text(M+35, hy+20, ebText, {size:12, bold:true, color:'#D7DBFA', ls:2}));
hy += 30 + 22;
// title — two fixed lines, all bold; measured with bold metrics (contiguous runs)
const tLines = [
  [{t:'SCN2A', col:'#FFFFFF'}],
  [{t:'related disorders', col:'#C7CBF7'}],
];
...
hy += tSize + tLH + 16;
// tagline (mixed)
const tag=[n('One gene.'), b(' One sodium channel — Nav1.2.'), n(' A spectrum from'), b(' self-limited seizures'), n(' to'), b(' severe, lifelong disability.')];
const tagR = rich(M, hy, 980, tag, {size:20, color:'#E9EAFC', boldColor:'#FFFFFF', lh:1.4});
headBuf.push(tagR.svg); hy += tagR.height + 12;
// definition
const def=[n('Variants in SCN2A alter the Nav1.2 neuronal sodium channel, causing a spectrum of epilepsy and neurodevelopmental disorders. Unusually, the type of variant predicts the phenotype, the age of onset, and even the response to treatment.')];
// left rule
const defR = rich(M+16, hy, 1000, def, {size:14, color:'#DDE0FA', boldColor:'#fff', lh:1.5});
headBuf.push(`<rect x="${M}" y="${r2(hy+2)}" width="3" height="${r2(defR.height-6)}" fill="rgba(255,255,255,0.5)"/>`);
headBuf.push(defR.svg); hy += defR.height + 22;
// glance pills
const pills=[
  {ic:'dna',      big:'Nav1.2 channel',  small:'neuronal sodium channel'},
  {ic:'spectrum', big:'A spectrum',      small:'seizures → autism / ID'},
  {ic:'pill',     big:'Genotype-guided', small:'variant predicts treatment'},
  {ic:'clock',    big:'Lifelong',        small:'24/7 support (severe end)'},
];
const pillGap=14, pillW=(CW-pillGap*3)/4, pillH=62;
pills.forEach((p,i)=>{
  const px=M+i*(pillW+pillGap);
  headBuf.push(rrect(px,hy,pillW,pillH,14,'rgba(255,255,255,0.14)','rgba(255,255,255,0.26)',1));
  headBuf.push(icon(p.ic, px+16, hy+(pillH-22)/2, 22, '#EEF0FE',1.9));
  headBuf.push(text(px+50, hy+28, p.big, {size:15, bold:true, color:'#FFFFFF'}));
  headBuf.push(text(px+50, hy+45, p.small, {size:11.5, color:'#CFD4F4'}));
});
hy += pillH + 38;
const headerH = hy;
...
push(`<defs>
  <linearGradient id="hg" x1="0" y1="0" x2="1" y2="1">
    <stop offset="0" stop-color="#3A33A8"/><stop offset="0.5" stop-color="#4F46E5"/><stop offset="1" stop-color="#6366F1"/>
  </linearGradient>
  ...
  <linearGradient id="spec" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="${C.gof}"/><stop offset="0.5" stop-color="#A8AEC8"/><stop offset="1" stop-color="${C.lof}"/></linearGradient>
...
/* ---------------- SECTION 1 : THE SCN2A SPECTRUM ---------------- */
sectionHead(1, 'One gene — a spectrum of disorders',
  'SCN2A variants change how the Nav1.2 sodium channel works. Whether the channel becomes over-active or under-active shapes the disorder, its age of onset, and even which medicines help.');

// spectrum bar with two poles
const hw = W/2;
push(rrect(M, y, CW, 16, 8, 'url(#spec)'));
push(`<circle cx="${hw}" cy="${r2(y+8)}" r="9" fill="#fff" stroke="#A8AEC8" stroke-width="4"/>`);
push(text(M, y+38, 'GAIN-OF-FUNCTION', {size:12, bold:true, color:C.gof, ls:1}));
push(text(M, y+54, 'channel over-active → too much signal', {size:12, color:C.muted}));
push(text(hw, y+38, 'severe DEE', {size:11.5, bold:true, color:C.inkSoft, anchor:'middle'}));
push(text(hw, y+54, 'across the middle', {size:11, color:C.muted, anchor:'middle'}));
push(text(W-M, y+38, 'LOSS-OF-FUNCTION', {size:12, bold:true, color:C.lof, anchor:'end', ls:1}));
push(text(W-M, y+54, 'channel under-active → too little signal', {size:12, color:C.muted, anchor:'end'}));
y += 16 + 66;

// two spectrum cards
const specCards=[
  {c:C.gof,t:C.gofT,ic:'zap',title:'Gain-of-function',tag:'EARLY-ONSET EPILEPSY',items:[
    [b('Seizures usually begin before 3 months'),n(' of age, often severe.')],
    [b('Ohtahara & West syndromes,'),n(' EIEE, and migrating focal seizures of infancy.')],
    [n('At the mild end: '),b('self-limited familial neonatal-infantile seizures.')],
    [b('Often responds to sodium-channel blockers'),n(' (e.g. phenytoin, carbamazepine).')],
  ]},
  {c:C.lof,t:C.lofT,ic:'down',title:'Loss-of-function',tag:'AUTISM & INTELLECTUAL DISABILITY',items:[
    [b('Autism and/or intellectual disability'),n(' is the core feature.')],
    [b('Seizures, if any, begin at or after 3 months'),n(' — or never.')],
    [n('Can include '),b('episodic ataxia'),n(' and movement difficulties.')],
    [b('Sodium-channel blockers usually don’t help'),n(' — and may worsen seizures.')],
  ]},
];
const specGap=24, specCardW=(CW-specGap)/2, specColX=[M, M+specCardW+specGap];
const specTxtW=specCardW-20-24-18;
function specCardH(card){ let h=18+52+10; card.items.forEach(it=>h+=richHeight(specTxtW,it,13.5,1.4)+11); return h+6; }
const specH=Math.max(specCardH(specCards[0]),specCardH(specCards[1]));
specCards.forEach((card,i)=>{
  const x=specColX[i], yTop=y;
  push(rrect(x,yTop,specCardW,specH,18,'#FFFFFF',C.line,1));
  push(`<clipPath id="spc${i}"><rect x="${r2(x)}" y="${r2(yTop)}" width="${specCardW}" height="${r2(specH)}" rx="18"/></clipPath>`);
  push(`<g clip-path="url(#spc${i})"><rect x="${r2(x)}" y="${r2(yTop)}" width="${specCardW}" height="6" fill="${card.c}"/></g>`);
  ...
  card.items.forEach(it=>{
    const rr=rich(x+20+24,by,specTxtW,it,{size:13.5,color:C.inkSoft,boldColor:C.ink,lh:1.4});
    push(`<rect x="${r2(x+20+4)}" y="${r2(by+6)}" width="9" height="9" rx="3" fill="${card.c}"/>`);
    push(rr.svg); by+=rr.height+11;
  });
});
y += specH + 22;

// precision-medicine caption
const pcap=[b('The variant predicts the treatment. '),n('This genotype–phenotype link makes SCN2A a leading target for precision, channel-specific therapies.')];
const pcapInner=CW-58;
const pcapH=richHeight(pcapInner,pcap,14,1.45)+24;
push(rrect(M,y,CW,pcapH,14,C.bg1,C.line,1));
push(icon('pill', M+18, y+(pcapH-22)/2, 22, C.brand,1.8));
push(rich(M+48,y+12,pcapInner,pcap,{size:14,color:C.inkSoft,boldColor:C.ink,lh:1.45}).svg);
y += pcapH + 12;
push(text(M, y+10, 'Clinical literature: Wolff et al., Brain 2017; Sanders et al., Trends in Neurosciences 2018.', {size:11, color:C.muted}));
y += 24;
push(`<line x1="${M}" y1="${r2(y)}" x2="${W-M}" y2="${r2(y)}" stroke="${C.line}" stroke-width="1"/>`);
y += 40;

/* ---------------- SECTION 2 : ACROSS SYSTEMS ---------------- */
sectionHead(2, 'Disability across body systems',
  'At the severe end of the spectrum — developmental & epileptic encephalopathy — SCN2A affects the whole child, with impairments across every body system at once.');
...
y = cy + RAD + 52;
// caption
const capSegs=[b('Co-occurring, not isolated. '), n('At the severe end of the SCN2A spectrum, most children live with several of these disabilities at once — the reason their care is so complex.')];
...
sectionHead(3, 'Disability across the lifespan',
...
/* timeline */
const stages=[
  {c:C.cog, age:'INFANCY', h:'Onset varies by variant', items:['Gain-of-function: early seizures','Loss-of-function: autism / delay first','Feeding & sleep difficulties']},
...
/* ---------------- FOOTER ---------------- */
const footTop=y;
const noteSegs=[n('Educational summary of SCN2A-related disorders, compiled from clinical literature (Wolff 2017; Sanders 2018). Presentation varies widely by variant and individual; this is not medical advice.')];
const noteW=560;
...
push(icon('dna', M+9, fcy-12, 24, '#fff',1.8));
push(text(M+58, fcy-2, 'SCN2A-related disorders', {size:15, bold:true, color:C.ink}));
push(text(M+58, fcy+16, 'Caused by variants in the SCN2A (Nav1.2) gene', {size:12.5, color:C.muted}));
...
const dest = process.argv[2] || path.join(__dirname, '..', 'assets', 'scn2a-impact.png');
