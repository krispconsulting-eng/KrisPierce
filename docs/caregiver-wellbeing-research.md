# Caregiver Wellbeing — Literature Review & Resource Brief

Status: living document. Compiled to inform a **caregiver Wellness Wheel**
feature. This is a plain-language synthesis of published statistics and public
resources, not clinical advice. Figures are US-centric unless noted; treat them
as directional for an Australian family (the SCN2A/disability-caregiver context
of this project) rather than exact local rates.

## Why this matters here

This repo's charity ledger (`entity: charity`) is for **SCN2A** — a severe
genetic epilepsy / neurodevelopmental condition. The families behind it are
**parent-caregivers of a child with a rare, lifelong, high-needs disability**.
That is the highest-burden caregiving profile in the literature: caring for
someone with *both* a brain-health and a physical condition produces the
greatest burden, loneliness and isolation of any caregiver group
([Wiley/GPS, 2022](https://onlinelibrary.wiley.com/doi/full/10.1002/gps.5734)).
A wellbeing tool for this audience should assume high baseline strain, chronic
(not episodic) demand, and very little discretionary time.

## The framework: an 8-dimension Wellness Wheel

The most widely-used, public-domain model is **SAMHSA's Eight Dimensions of
Wellness** — Emotional, Physical, Social, Intellectual, Spiritual,
Environmental, Financial, and Occupational. All eight are interconnected;
progress in one lifts the others, and neglect in one drags the others down
([SAMHSA poster, SMA16-4953](https://library.samhsa.gov/sites/default/files/sma16-4953.pdf);
[SAMHSA, *Creating a Healthier Life*](https://library.samhsa.gov/product/creating-healthier-life-step-step-guide-wellness/sma16-4958)).
Using this model keeps the wheel evidence-based, avoids reinventing categories,
and lets each spoke map to a self-assessment prompt plus targeted resources.

> **Design implication.** Build the wheel as 8 spokes. Each spoke = (1) a
> short self-rating, (2) a one-line "why this matters" stat, (3) one small
> actionable step, (4) 1–2 vetted resources. Keep actions *tiny* — the core
> constraint for this audience is time, not motivation.

## Headline statistics (the case for the wheel)

- **Scale.** ~53 million US adults (>1 in 5) are unpaid family caregivers — up
  9.5 million in five years
  ([AARP/NAC, *Caregiving in the U.S. 2020*](https://www.caregiving.org/wp-content/uploads/2021/01/full-report-caregiving-in-the-united-states-01-21.pdf)).
- **Self-reported health decline.** 23% say caregiving has made their own
  health worse — up from 17% in 2015 (AARP/NAC 2020).
- **Complexity is rising.** 24% now care for more than one person; 26% report
  difficulty coordinating care (AARP/NAC 2020).

These are the "why now" numbers for a landing/intro screen.

## Per-dimension evidence and resource hooks

### 1. Emotional / Mental
- Caregivers are diagnosed with depression at **25.8%** vs **19.5%** in the
  general population (CDC BRFSS 2021, reported in
  [Forbes/Otsuka, 2025](https://www.forbes.com/sites/otsuka/2025/10/02/caregivings-mental-health-crisis/)).
- In the closest-matching group — **parents of children with disabilities** — a
  Japanese national survey found **44.4%** in psychological distress and **8.9%**
  with possible serious mental illness
  ([Miyamoto et al., *IJERPH* 2015, PMC4686893](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4686893/)).
- Parental burnout is markedly higher for parents of children with special needs
  / chronic disease, driven by prolonged care and neglect of the parent's own
  needs; higher depression/anxiety → higher burnout
  ([MDPI/*EJIHPE* 2024, PMC11592539](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC11592539/)).
- **Resource hooks:** 988 Suicide & Crisis Lifeline (24/7); Caregiver Action
  Network Family Caregiver Help Desk (1-855-227-3640). *(Substitute local
  equivalents for an AU audience — e.g. Lifeline 13 11 14, Carer Gateway
  1800 422 737.)*

### 2. Physical
- Chronic caregiver stress and social isolation are linked to higher rates of
  heart disease, hypertension, and immune suppression
  ([Sailor Health](https://www.sailorhealth.com/blog-post/caregiver-loneliness);
  [Child Mind Institute](https://childmind.org/article/fighting-caregiver-burnout-special-needs-kids/)).
- **Sleep** is a first-casualty domain: overnight monitoring, seizures, and
  waking children fragment caregiver sleep, and loneliness worsens it
  ([Nature *Sci Reports* 2021, PMC7921547](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7921547/)).
- Objective marker: caregiver burden in parents of children with special
  healthcare needs is associated with elevated **inflammation**
  ([*Discover Psychology* 2023](https://link.springer.com/article/10.1007/s44202-023-00089-z)).
- **Wheel action idea:** protect one sleep block / arrange overnight respite;
  attach preventive-health check reminders (the caregiver's *own* GP visits).

### 3. Social
- ~**10%** of family caregivers feel lonely "often or always," and those who do
  report more physical *and* mental health problems
  ([*IJERPH* 2022, PMC9222605](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9222605/)).
- Low social support is the factor most consistently tied to caregiver
  psychological distress — i.e. it's **modifiable** (PMC4686893, above).
- **Resource hooks:** disease-specific peer communities (e.g. SCN2A family
  networks / foundations), Area Agency on Aging / local carer groups, condition
  Facebook groups. Peer support is one of the highest-leverage, lowest-cost
  interventions.

### 4. Financial
- Nearly **8 in 10** caregivers have routine out-of-pocket costs, averaging
  **$7,242/yr** — about **26% of income**
  ([AARP, 2021 Out-of-Pocket Costs Study](https://www.aarp.org/caregiving/financial-legal/high-out-of-pocket-costs/)).
- ~**1 in 5** report **high** financial strain; roughly half report a financial
  setback (AARP 2021).
- Strain falls hardest on younger, lower-income, and Hispanic/Latino and
  Black caregivers (AARP 2021).
- **This is the wheel's natural tie-in to the rest of the app** — the existing
  Fee Finder / Subscriptions / Debt Payoff tooling *is* a financial-wellbeing
  intervention. Link the Financial spoke straight into those views.

### 5. Occupational (work / role)
- ~**59%** of caregivers work; **56%** go in late / leave early / take time off,
  **18%** cut hours, and **9%** stop working entirely — an average
  **$10,525/yr** in work-related costs
  ([AARP Out-of-Pocket Costs Study](https://www.aarp.org/pri/topics/ltss/family-caregiving/family-caregivers-cost-survey/)).
- **Wheel action idea:** surface employer carer-leave entitlements and flexible-
  work rights; for AU, note Carer Leave provisions and the role a diagnosis
  letter plays in workplace adjustments.

### 6. Intellectual
- 26% of caregivers struggle to *coordinate* care (AARP/NAC 2020) — the load is
  cognitive as well as emotional. Learning the condition, navigating systems,
  and organising appointments are real work.
- **Wheel action idea:** a "learn one thing" prompt + curated explainers;
  care-coordination checklists; a single place for the child's medical summary.

### 7. Environmental
- Home environment and equipment (safe sleep/seizure setup, accessibility,
  reducing overnight-monitoring burden) directly affect both the caregiver's
  sleep and stress load — this dimension is practical, not abstract, for
  high-needs pediatric care.
- **Wheel action idea:** home-setup / equipment checklist; respite-space and
  local-services locator.

### 8. Spiritual (meaning & purpose)
- SAMHSA frames this as sense of purpose and meaning, not religion specifically
  ([SAMHSA](https://library.samhsa.gov/sites/default/files/sma16-4953.pdf)).
  For rare-disease parents, connection to advocacy/community and meaning-making
  are documented protective, burnout-buffering resources (PMC11592539).
- **Wheel action idea:** reflection prompts; connection to the SCN2A charity's
  own mission as a source of meaning.

## Evidence-based interventions to point the wheel at

- **Respite care** — short-term relief; the single most-requested support. US
  programs delivered ~6M hours to 604k+ caregivers; find local via the
  Eldercare Locator model. *(AU: Carer Gateway funds respite.)*
  ([ACL National Family Caregiver Support Program](https://acl.gov/programs/support-caregivers/national-family-caregiver-support-program))
- **Peer / support groups** — targets the highest-leverage modifiable factor
  (social support).
- **Psychoeducation + skills** (stress management, problem-solving, sleep
  hygiene) — the components behind measured burnout reduction (PMC11592539).
- **Financial navigation** — this app's existing tooling.

## Vetted starting resources

| Need | Resource |
|---|---|
| General caregiver support & help desk | [Caregiver Action Network](https://www.caregiveraction.org/) — 1-855-227-3640 |
| Fact sheets, care planning, helpline | [Family Caregiver Alliance](https://www.caregiver.org/) |
| Wellness framework | [SAMHSA — 8 Dimensions of Wellness](https://library.samhsa.gov/sites/default/files/sma16-4953.pdf) |
| Respite & local services (US) | [ACL / Eldercare Locator](https://acl.gov/programs/support-caregivers/national-family-caregiver-support-program) |
| Special-needs parent burnout | [Child Mind Institute — Caregiver Burnout](https://childmind.org/article/fighting-caregiver-burnout-special-needs-kids/) |
| Mental-health crisis (US) | 988 Suicide & Crisis Lifeline |
| **AU equivalents to add** | Carer Gateway (1800 422 737), Lifeline (13 11 14), Beyond Blue, condition-specific SCN2A foundation/community |

> **Localisation note:** most hard statistics above are US (AARP/NAC, CDC). The
> *patterns* — depression ~1.3× population rate, high financial and work strain,
> social support as the key modifiable buffer — generalise well and are enough
> to design against. For any Australian rollout, swap in AIHW/Carers Australia
> figures and the AU helplines before publishing user-facing numbers.

## Recommended next step for the wheel

Ship the 8-spoke SAMHSA wheel with a short self-rating per spoke, one stat, one
tiny action, and the vetted links above — and wire the **Financial** spoke into
the app's existing Fee Finder / Subscriptions / Debt Payoff views so the wheel
isn't just informational but plugs into tools the family already uses.

## Sources

- AARP & National Alliance for Caregiving, *Caregiving in the U.S. 2020* — https://www.caregiving.org/wp-content/uploads/2021/01/full-report-caregiving-in-the-united-states-01-21.pdf
- AARP, *2021 Family Caregivers Cost (Out-of-Pocket) Study* — https://www.aarp.org/pri/topics/ltss/family-caregiving/family-caregivers-cost-survey/ and https://www.aarp.org/caregiving/financial-legal/high-out-of-pocket-costs/
- CDC BRFSS 2021 caregiver depression data, via Forbes/Otsuka (2025) — https://www.forbes.com/sites/otsuka/2025/10/02/caregivings-mental-health-crisis/
- Miyamoto et al., *Mental Health of Parents as Caregivers of Children with Disabilities* (Japan), IJERPH 2015 — https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4686893/
- *Parental Burnout — Risk Factors and Protective Resources* (mothers of children with/without special needs), 2024 — https://www.ncbi.nlm.nih.gov/pmc/articles/PMC11592539/
- Caregiver burden and inflammation in parents of children with special healthcare needs, *Discover Psychology* 2023 — https://link.springer.com/article/10.1007/s44202-023-00089-z
- Care burden, loneliness, and social isolation in caregivers (before/during COVID-19), *GPS* 2022 — https://onlinelibrary.wiley.com/doi/full/10.1002/gps.5734
- *Family Caregiver's Loneliness and Related Health Factors*, IJERPH 2022 — https://www.ncbi.nlm.nih.gov/pmc/articles/PMC9222605/
- Loneliness/isolation and sleep problems, *Scientific Reports* 2021 — https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7921547/
- Child Mind Institute, *Caregiver Burnout / special-needs kids* — https://childmind.org/article/fighting-caregiver-burnout-special-needs-kids/
- SAMHSA, *Learn the Eight Dimensions of Wellness* (SMA16-4953) & *Creating a Healthier Life* (SMA16-4958) — https://library.samhsa.gov/sites/default/files/sma16-4953.pdf
- Caregiver Action Network — https://www.caregiveraction.org/
- Family Caregiver Alliance — https://www.caregiver.org/
- ACL, National Family Caregiver Support Program — https://acl.gov/programs/support-caregivers/national-family-caregiver-support-program
